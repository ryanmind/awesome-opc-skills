...
1. Search with the script before opening files:
   ...
   ```bash
   python3 skills/jaron-wiki/scripts/wiki_search.py --query "<query>" --scope formal --limit 8 --json
   ```
...
3. Read only the smallest sufficient pages:
   ...
   ```bash
   python3 skills/jaron-wiki/scripts/wiki_page.py "<slug-or-path>" --max-chars 12000 --json
   ```
